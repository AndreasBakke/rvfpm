...
void resetRegisters(FPU& fpu, std::vector<float>& clone){
    ...
    for (unsigned int i = 0; i < rfDepth; i++)
    {
        ...
        fpu.operation(instr_load.instr, 0, w_data, nullptr, nullptr, nullptr);
    }
}
...
void verifyReset(FPU& fpu, std::vector<float>& clone){
    ...
    for (unsigned int i = 0; i < rfDepth; i++)
    {
        ...
        FpuPipeObj res = fpu.operation(instr_read.instr, 0, 0, &toMem, nullptr, nullptr);
        if (toMem != clone[i])
        {
...
void testR4TYPE(FPU& fpu, std::vector<float>& clone) {
    for (int i = 0; i < numTests; i++) //do numTests random tests
    {
        ...
        float d1 = fpu.operation(instr_read_1.instr, 0, 0, nullptr, nullptr, nullptr).data.f;
        STYPE instr_read_2 = {.parts= {39, 0, 0b010, 0, r2, 0}};
        float d2 = fpu.operation(instr_read_2.instr, 0, 0, nullptr, nullptr, nullptr).data.f;
        ...
        fpu.operation(instr_r4type.instr, 0, 0, nullptr, nullptr, nullptr); //do operation

        //Read and compare
        STYPE instr_read = {.parts= {39, 0, 0b010, 0, rd, 0}};
        float res = fpu.operation(instr_read.instr, 0, 0, nullptr, nullptr, nullptr).data.f;
...
void testRTYPE(FPU& fpu, std::vector<float>& clone) {
    for (int i = 0; i < numTests; i++) //do numTests random tests 
    {
        ...
        fpu.operation(instr_rtype.instr, 0, 0, nullptr, nullptr, nullptr); //do operation
        //Read and compare
        STYPE instr_read = {.parts= {39, 0, 0b010, 0, rd, 0}};
        float res = fpu.operation(instr_read.instr, 0, 0, nullptr, nullptr, nullptr).data.f;
