...
floatType hexToFloat(const std::string& hexStr) {
...
std::string floatToHex(floatType floatValue) {
  ...
  auto value = *reinterpret_cast<unsignedType*>(&floatValue);

  // Set the width to 8 characters and fill with zeroes
  oss << std::setw(w) << std::setfill('0') << value;
...
unsignedType hexToUnsignedInt(const std::string& hexStr) {
  unsignedType value;
...
signedType hexToInt(const std::string& hexStr) {
  signedType value;
  std::stringstream ss(hexStr);
  ss >> std::hex >> value;
  return value;
...
std::string uint32ToHexString(unsignedType uintValue) {
  // Create an output string stream with hexadecimal formatting and no showbase
  std::ostringstream oss;
  oss << std::hex << std::noshowbase;

  // Interpret the bits of the float as an unsigned integer
  auto value = *reinterpret_cast<unsignedType*>(&uintValue);
...
int main(int argc, char** argv) {
  ...
  if (op == "fmadd" || op == "fmsub" || op == "fnmsub" || op == "fnmadd") //Fused operations has an extra input compared to others
  {
    while (std::cin >> input1 >> input2 >> input3 >> input4 >> flags) {
      unsignedType a = hexToUnsignedInt(input1);
      unsignedType b = hexToUnsignedInt(input2);
      unsignedType c = hexToUnsignedInt(input3);
      unsigned int r1 = 1;
      unsigned int r2 = 2;
      unsigned int r3 = 3;
    ...
    while (std::cin >> input1 >> input2 >> flags) {
      ...
      if (op == "fsqrt" || op == "ui32_to_f32" || op == "i32_to_f32") {
        ...
        std::cout << input1 << " " << uint32ToHexString(static_cast<unsignedType>(result.data.s)) << " " << convFlags(result.flags & 0b01111)  << std::endl; //Conversion to int doesn't check for exactness
      }
    }
  } else {
