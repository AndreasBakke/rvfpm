...
extern "C" {
    ...
    void fpu_operation(void* fpu_ptr, uint32_t instruction, unsigned int id, uint32_t fromXReg, float fromMem, unsigned int* id_out, float* toMem, uint32_t* toXReg, bool* pipelineFull, bool* toMem_valid, bool* toXReg_valid){ //data only passed for operations using int(X)-registers
